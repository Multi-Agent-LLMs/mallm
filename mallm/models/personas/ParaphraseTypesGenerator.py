...
class ParaphraseTypesGenerator(PersonaGenerator):
    ...
    def generate_personas(
        ...
        paraphrase_types_list = [
            ptype.strip().lower()
            for ptype in context.split(",")
            if ptype.strip().lower() in self.paraphrase_types.keys()
        ]  # excludes extremes identity, non-paraphrase, and entailment
        return [
            {
                "role": "Expert in " + p,
                "description": self.paraphrase_types[p]
                + " Make sure your paraphrase type is properly used for the solution.",
            }
