...
class MajorityConsensus():
    ...
    def decide(self, agreements, turn):
        if len(self.panelists) <= 3 and turn < 5:
            return sum(agreements) == len(self.panelists)  # all agents need to agree in the first 5 turns (except moderator)
        else:
            return sum(agreements) > len(self.panelists)/2    # more than half of the agents need to agree (except moderator)
