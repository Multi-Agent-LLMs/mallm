...
def main(data, instruction, use_moderator=False, max_turns=10, feedback_sentences=[3, 4], context_length=1,
    ...
    for p in PARADIGMS:
        run.main(data=data, out=f"experiments/out/exp1/exp1_{data.split('/')[-1].split('.')[0]}_{p}.json",
                 instruction=instruction, use_moderator=False, max_turns=10, feedback_sentences=[3, 4],
                 paradigm="memory", context_length=1, include_current_turn_in_memory=False)
