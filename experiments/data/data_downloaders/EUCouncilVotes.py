...
class EUCouncilVotesDownloader(DatasetDownloader):
    def custom_download(self):
        ...
        definitions = [
            x
            for x in voting_results_cube
            if "http://www.w3.org/2004/02/skos/core#definition" in x
        ]
        for act in definitions:
            ...
            votes = [
                x
                for x in voting_results_cube
                if "http://data.consilium.europa.eu/data/public_voting/qb/dimensionproperty/act"
                in x
                and "@id"
                in x[
