...
def test_unanimous_decision_in_first_five_turns():
    ...
    agreements = [Agreement(agreement=True, agent_id="", persona="", response="") for _ in range(3)]
...
def test_no_unanimous_decision_in_first_five_turns():
    mc = MajorityConsensus(panelists[:3])
    agreements = [Agreement(agreement=True, agent_id="", persona="", response="") for _ in range(2)]
...
def test_majority_decision_after_five_turns():
    ...
    agreements = [Agreement(agreement=True, agent_id="", persona="", response="") for _ in range(3)]
    agreements.extend([Agreement(agreement=False, agent_id="", persona="", response="") for _ in range(2)])
...
def test_no_majority_decision_after_five_turns():
    mc = MajorityConsensus(panelists)
    agreements = [Agreement(agreement=True, agent_id="", persona="", response="") for _ in range(2)]
    agreements.extend([Agreement(agreement=False, agent_id="", persona="", response="") for _ in range(3)])
    decision, is_consensus = mc.make_decision(agreements, 6, "", "")
    assert not is_consensus
